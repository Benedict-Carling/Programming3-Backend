...
router.get("/table", async(req,res) => {//GET request endpoint to retrieve all the data entries from the data table
    try {
...
router.post("/add-comment", async (req,res) =>{//POST request endpoint to edit the data table with user input
  ...
  try {
      ...
      await Data.updateOne({Id:InputId},//specifically updating the those fields that would have been edited
      {$set:{
...
router.post("/show-image",async (req,res) =>{/*POST request endpoint for accessing images from database storage
